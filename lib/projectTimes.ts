...
export const projectTimes = {
	...
	calculateProjectMetrics(tasks: Task[]) {
		...
		return {
			...
			duration: Math.ceil(
				(Math.max(...projectTasks.map((t) => t.endDate)) -
					Math.min(...projectTasks.map((t) => t.startDate))) /
					(24 * 60 * 60 * 1000)
			),
			progress:
				projectTasks.reduce(
	...
	recalculateProjectDates(project: Task, tasks: Task[]) {
		...
		return projectTasks.reduce((acc, task) => {
			...
			const dates = this.calculateTaskDates(
				task,
				project.startDate,
				dependencies
			)
	...
	getAffectedTasks(taskId: string, tasks: Task[]): Task[] {
		...
		return [
			...dependentTasks,
			...dependentTasks.flatMap((t) =>
				this.getAffectedTasks(t.id, tasks)
			),
		]
	},
}
