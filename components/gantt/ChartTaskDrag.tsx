...
interface ChartTaskDragProps {
  ...
  startDate: Date
  onDragEnd: (taskId: string, newStartDate: Date, newEndDate: Date, newOrder: number) => void
  className?: string
}
...
export function ChartTaskDrag({
  ...
  const originalPosition = useRef({ startDate: task.startDate, order: task.order })

  const handleDragStart = (e: React.MouseEvent) => {
    e.preventDefault()
    setIsDragging(true)
    dragStart.current = { x: e.clientX, y: e.clientY }
    originalPosition.current = { startDate: task.startDate, order: task.order }
  }
  ...
  const handleDrag = (e: React.MouseEvent) => {
    ...
    const newStartDate = addDays(originalPosition.current.startDate, daysDelta)
  ...
  return (
    <div
      ...
      style={{
        left: dimensions.LABEL_WIDTH + task.order * dimensions.DAY_WIDTH,
        top: dimensions.HEADER_HEIGHT + task.order * dimensions.ROW_HEIGHT,
        width: differenceInDays(task.endDate, task.startDate) * dimensions.DAY_WIDTH,
